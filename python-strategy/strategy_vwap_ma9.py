...
try:
    ...
    def sync_candles_to_strapi(ticker: str, df: pd.DataFrame, symbol_id: str, timeframe: str = "D1", max_sync: int = 50000):
        sync_candles_to_strapi_bulk(ticker, df, symbol_id=symbol_id, timeframe=timeframe, max_sync=max_sync)
except ImportError:
    ...
    def sync_candles_to_strapi(ticker: str, df: pd.DataFrame, symbol_id: str, timeframe: str = "D1", max_sync: int = 1500):
        """Đồng bộ các nến từ external vào bảng symbol-histories của Strapi"""
        ...
        try:
            ...
            requests.post(
                ...
                json={"symbolId": symbol_id, "symbol": clean, "timeframe": tf, "candles": candles_payload},
                headers=headers,
                timeout=12
            )
        except Exception:
        ...
        try:
            ...
            if res.status_code == 200:
                ...
                for i in range(len(timestamps)):
                    ...
                    if ts is None or c is None or pd.isna(c) or float(c) <= 0:
                        continue
                    o = opens[i] if i < len(opens) and not pd.isna(opens[i]) and float(opens[i]) > 0 else c
                    h = highs[i] if i < len(highs) and not pd.isna(highs[i]) and float(highs[i]) > 0 else max(o, c)
                    l = lows[i] if i < len(lows) and not pd.isna(lows[i]) and float(lows[i]) > 0 else min(o, c)
                ...
                if candles:
                    df = pd.DataFrame(candles)
                    cleaned = clean_candle_df(df)
                    if not cleaned.empty and len(cleaned) > 0:
                        return cleaned
        except Exception:
            pass
    return pd.DataFrame()
...
def optimize_strategy_parameters(
    ...
    if all_candidates:
        ...
        for c in all_candidates:
            key = (c["maPeriod"], c["tpTarget"], c["mult2"], c["mult3"], c["vwapAnchor"])
            if key not in seen:
                seen.add(key)
